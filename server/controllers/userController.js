...
// import axios from 'axios'; // Not used with Firebase Frontend flow
// import OtpModel from "../models/Otp.js"; // Not used
...
export const register = async (req, res) => {
    try {
        ...
        // Check if user already exists (Mobile check)
        // Since Firebase verified the mobile, we trust it belongs to the user trying to register.
        const existingUserMobile = await User.findOne({ mobile });
        if (existingUserMobile) return res.json({ success: false, message: 'Mobile number already registered' });
...
// Mobile Login (Firebase verified on Frontend) : /api/user/login-mobile
export const loginWithMobile = async (req, res) => {
    try {
        const { mobile } = req.body;

        if (!mobile) return res.json({ success: false, message: "Mobile and OTP required" });

        // NOTE: In a production app, verify the Firebase ID Token here!
        // For now, we trust the frontend has verified the user.
