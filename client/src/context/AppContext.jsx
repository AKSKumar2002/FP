...
export const AppContextProvider = ({ children }) => {
  ...
  const fetchSeller = async () => {
    try {
      const { data } = await axiosInstance.get("http://localhost:4000/api/seller/is-auth");
  ...
  const fetchUser = async () => {
    try {
      const { data } = await axiosInstance.get("http://localhost:4000/api/user/is-auth");
      if (data.success) {
        setUser(data.user);
  ...
  const fetchProducts = async () => {
    try {
      const { data } = await axiosInstance.get("http://localhost:4000/api/product/list");
      if (data.success) {
  ...
  useEffect(() => {
    const updateCart = async () => {
      try {
        const { data } = await axiosInstance.post("http://localhost:4000/api/cart/update", {
          userId: user._id,
          cartItems,
        });
        if (!data.success) {
